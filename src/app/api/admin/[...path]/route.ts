...
import { ACCESS_COOKIE, REFRESH_COOKIE, getAdminApiBaseUrl, refreshBackendSession } from "@/lib/admin/session";

// Proxy chung cho mọi request admin -> backend Mini-OTA (Spring Boot).
// Lý do cần proxy thay vì FE gọi thẳng: backend chưa cấu hình CORS, và ta
// không muốn access token nằm trong JS phía client (cookie httpOnly an toàn
// hơn) — xem trao đổi lúc thiết kế.
async function forward(req: NextRequest, path: string[], accessToken: string, bodyText: string | undefined) {
  const url = `${getAdminApiBaseUrl()}/${path.join("/")}${req.nextUrl.search}`;
